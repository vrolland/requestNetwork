...
async function sign(
  ...
  if (signatureParams.method === SignatureTypes.METHOD.EDDSA_POSEIDON) {
    ...
    return {
      data,
      signature: { method: signatureParams.method, value: value.concat(pubKey) },
      raw: rawSignature,
    };
  }
...
function recoverSigner(signedData: SignatureTypes.ISignedData): IdentityTypes.IIdentity {
  ...
  if (signedData.signature.method === SignatureTypes.METHOD.EDDSA_POSEIDON) {
    ...
    const dataToVerify = signedData.raw
      ? signedData.data
      : normalizeKeccak256Hash(signedData.data).value;

    const verified = edVerify(packedSignatureHex, dataToVerify, pubkeyHex);

    if (verified) {
      return {
        type: IdentityTypes.TYPE.POSEIDON_ADDRESS,
        value: getAddressFromEdPublicKey(pubkeyHex),
      };
    } else {
