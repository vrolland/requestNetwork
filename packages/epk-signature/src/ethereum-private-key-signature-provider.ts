...
export default class EthereumPrivateKeySignatureProvider
  ...
  public supportedMethods: SignatureTypes.METHOD[] = [
    ...
    SignatureTypes.METHOD.EDDSA_POSEIDON,
  ];
  ...
  public async sign(
    ...
    const method =
      signer.type === IdentityTypes.TYPE.POSEIDON_ADDRESS
        ? SignatureTypes.METHOD.EDDSA_POSEIDON
        : SignatureTypes.METHOD.ECDSA;
  ...
  public addSignatureParameters(
    ...
    // toLowerCase to avoid mismatch because of case
    const identity = getIdentityFromSignatureParams(signatureParams);

    this.signatureParametersDictionary.set(identity.value.toLowerCase(), signatureParams);
  ...
  public getAllRegisteredIdentities(): IdentityTypes.IIdentity[] {
    return Array.from(this.signatureParametersDictionary.keys(), (address) => ({
      type:
        this.signatureParametersDictionary.get(address)?.method === SignatureTypes.METHOD.ECDSA
          ? IdentityTypes.TYPE.ETHEREUM_ADDRESS
          : IdentityTypes.TYPE.POSEIDON_ADDRESS,
      value: address,
    }));
